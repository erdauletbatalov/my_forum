...
func (m *ForumModel) GetLikedOrDislikedUserPosts(session_user_id int, by_user int, vote_type int) ([]*models.Post, error) {
	stmt := `SELECT u.username, p.user_id, p.id, p.title, p.content, p.date
					FROM post AS p
					INNER JOIN user AS u
					ON p.user_id = u.id
					...
					ON v.post_id = p.id
					WHERE v.comment_id = 0
					AND v.user_id = ?
					AND v.vote_type = ?`

	rows, err := m.DB.Query(stmt, by_user, vote_type)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
